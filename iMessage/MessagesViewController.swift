...
class MessagesViewController: MSMessagesAppViewController {
    ...
    // Remove any child view controllers that have been presented.
        ...
        self.add(swiftUIView: AnyView(MessageView(action: { image, postAsImage in
            if postAsImage {
                self.post(image: image)
            } else {
                self.post(caption: "This is cool", for: image)
            }
        })))
    }
    
    func post(caption: String, for image: Image) {
    ...
    func post(image: Image) {
        ...
        let uiImage = image.getUIImage(newSize: .init(width: 100, height: 100))
        print(paths)
